#!/usr/bin/env bash
# Train the corrected LIBERO Past2Next baseline with an existing frozen tokenizer.
set -euo pipefail

usage() {
    cat <<'HELP'
Usage: bash train_baseline.sh [--dry-run] TOKENIZER_CHECKPOINT [OUTPUT_DIR]

Uses train_past2next_scratch with model seed 42 and dataset split seed 42.
...
RUN_DIR=$(realpath -m -- "${2:-$REPO_DIR/output/training/baseline_corrected_seed42_$(date -u +%Y%m%d_%H%M%S)}")
...
ARGS=(
    --config-name=train_past2next_scratch
