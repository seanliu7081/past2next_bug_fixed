#!/usr/bin/env bash
# Single latent-flow Bash entrypoint: online W&B and sequential plain/gate runs.
set -euo pipefail
...
flow_help() {
  cat <<'HELP'
Usage: bash train_p2n_latent_flow.sh [OPTIONS] [-- HYDRA_OVERRIDES...]
...
if [[ -n "$FLOW_RESUME" && "$FLOW_SELECTION" == both ]]; then
  echo '--resume requires --variant p2n_latent_flow or --variant p2n_state_gate_latent_flow' >&2
  exit 2
fi
