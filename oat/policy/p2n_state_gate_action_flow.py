...
from oat.policy.p2n_action_flow_common import P2NActionFlowCommonPolicy, bool_mask
from oat.model.action_flow_state_history import Rotation6DStateActionHistoryEncoder


class P2NStateGateActionFlowPolicy(P2NActionFlowCommonPolicy):
    ...
    def build_context(self, obs, past_actions, past_action_valid, frozen_patches=None):
        ...
        base = super().build_context(obs, past_actions, valid, frozen_patches)
        ...
        visual = visual.reshape(visual.shape[0], self.n_obs_steps, len(self.obs_encoder.rgb_ports),
                                self.obs_encoder.resampler.num_queries, self.obs_feature_dim)
