"""Independent direct-action-flow training, strict artifacts, and unbiased validation.
...
METRICS = ("fm_loss", "action_mse", "normalized_action_mse", "translation_mse",
           "rotation_mse", "gripper_mse", "normalized_out_of_bounds")
...
def stable_validation_seed(seed: int, dataset_identity: str, sample_id: int) -> int:
    ...
    payload = f"direct-action-flow-validation-v1\0{seed}\0{dataset_identity}\0{sample_id}".encode()
...
def assert_frozen_normalizer(policy):
    """Affine action/state statistics are fixed model state, never optimized."""
    normalizer = policy.action_normalizer
    if any(parameter.requires_grad for parameter in normalizer.parameters()):
        raise ValueError("Action/state normalizer parameters must remain frozen")
    if any(not torch.isfinite(value).all() for value in normalizer.state_dict().values()):
        raise ValueError("Action/state normalizer must contain finite statistics")
    return _module_digest(normalizer)


def assert_equal_normalizers(student, teacher):
    if assert_frozen_normalizer(student) != assert_frozen_normalizer(teacher):
        raise ValueError("Student and EMA action/state normalizers must match exactly")
...
class TrainP2NActionFlowWorkspace(BaseWorkspace):
    include_keys = ("epoch", "global_step", "completed_optimizer_steps", "ema_state",
                    ...
                    "best_metric", "skipped_optimizer_steps", "normalizer_provenance")
    ...
    def __init__(self, cfg, output_dir=None, lazy_instantiation=True):
        ...
        self.normalizer_provenance = None
        self.best_metric = math.inf
        self.generators = {}
        if not lazy_instantiation:
            self.model = hydra.utils.instantiate(cfg.policy)
            self.optimizer = self.model.get_optimizer(**cfg.optimizer)
            # The teacher is created only after normalizer fitting and DDP sync.

    @staticmethod
    def validate_resume_payload(payload, cfg):
        metadata = payload.get("metadata", {})
        if metadata.get("policy_family") != POLICY_FAMILY:
            raise ValueError("Resume policy_family must be continuous_action_flow; AR artifacts are incompatible")
        if metadata.get("artifact_schema_version") != ARTIFACT_SCHEMA_VERSION:
            raise ValueError("Unsupported direct-action-flow artifact schema version")
        ...
        if not set(TrainP2NActionFlowWorkspace.include_keys).issubset(payload.get("pickles", {})):
        ...
        ignored = {"dino_path", "dino_revision", "construction_mode"}
        ...
        for section in ("ema", "optimizer", "normalization"):
    ...
    def _load_continuation(self, payload):
        ...
        if self.normalizer_provenance is None:
            raise ValueError("Resume requires training-frame normalizer provenance")
        digest = assert_frozen_normalizer(self.model)
        if self.normalizer_provenance.get("state_sha256") != digest:
            raise ValueError("Resume normalizer differs from its saved training-frame statistics")

    def _fit_fresh_normalizer(self, dataset, split):
        """Fit once through the dataset's original real-frame training mask."""
        configuration = self.cfg.get("normalization", {})
        if configuration.get("source", "training_replay_frames") != "training_replay_frames":
            raise ValueError("Direct action flow fits only training replay frames")
        if configuration.get("refit_on_resume", False):
            raise ValueError("Direct action-flow normalizers cannot be refitted on resume")
        mode = configuration.get("mode", "limits")
        if mode != "limits":
            raise ValueError("This action-flow recipe requires limits normalization")
        train_mask = np.asarray(dataset.train_mask, dtype=np.bool_)
        fitting_mask = np.asarray(dataset.normalization_train_mask, dtype=np.bool_)
        if not np.array_equal(train_mask, fitting_mask):
            raise ValueError("Normalizer fitting must use exactly the selected training episodes")
        episode_ends = np.asarray(dataset.replay_buffer.episode_ends, dtype=np.int64)
        lengths = np.diff(np.concatenate(([0], episode_ends)))
        if lengths.shape != train_mask.shape or np.any(lengths <= 0):
            raise ValueError("Invalid training replay episode boundaries")
        training_frames = int(lengths[train_mask].sum())
        if training_frames < 1:
            raise ValueError("Normalizer needs at least one real training frame")
        self.model.set_normalizer(dataset.get_normalizer(mode=mode))
        self.normalizer_provenance = {
            "schema_version": 1, "source": "training_replay_frames", "mode": mode,
            "dataset_identity": split["identity"],
            "train_episode_ids": list(split["train_episode_ids"]),
            "training_frames": training_frames,
            "state_sha256": assert_frozen_normalizer(self.model),
        }
        self.model.normalizer_metadata = copy.deepcopy(self.normalizer_provenance)
    ...
    def save_checkpoint(self, path=None, tag="latest", **_):
        ...
        for relative in ("oat/workspace/train_p2n_action_flow.py", "oat/dataset/action_flow_dataset.py",
                         "scripts/train_p2n_action_flow.py"):
        ...
        metadata.update(policy_family=POLICY_FAMILY, variant=self.cfg.variant,
                        ...
                        normalizer_provenance=copy.deepcopy(self.normalizer_provenance),
    ...
    def create_from_checkpoint(cls, path, output_dir=None, **_):
        ...
        instance.model.normalizer_metadata = copy.deepcopy(instance.normalizer_provenance)
        instance.ema_model.normalizer_metadata = copy.deepcopy(instance.normalizer_provenance)
        assert_equal_normalizers(instance.model, instance.ema_model)
        instance.model.eval().reset()
        instance.ema_model.eval().reset()
        return instance

    @torch.no_grad()
    def _validate(self, accelerator, loader, dataset, *, generated, compute_actions):
            ...
            for index in range(batch["action"].shape[0]):
                ...
                for mode in modes:
                    ...
                    with accelerator.autocast():
                        measured = policy.validation_metrics(sample, generator=generator, history_mode=mode, compute_actions=compute_actions)
        ...
        result["val_action_mse"] = values["expert_action_mse"]
    ...
    def run(self):
        ...
        if cfg.get("policy_family") != POLICY_FAMILY or cfg.variant not in VARIANTS:
            raise ValueError("This workspace only accepts explicit direct-action-flow variants")
        if not cfg.training.use_ema:
            raise ValueError("FM+CT training requires use_ema=true")
        if cfg.get("normalization", {}).get("refit_on_resume", False):
            raise ValueError("Direct action-flow normalizers cannot be refitted on resume")
        ...
        if payload is None:
            self._fit_fresh_normalizer(dataset, split)
        else:
        ...
        self.model.normalizer_metadata = copy.deepcopy(self.normalizer_provenance)
        assert_frozen_normalizer(self.model)
        ...
        assert_equal_normalizers(student, self.ema_model)
        ...
        while self.epoch < int(cfg.training.num_epochs):
            ...
            for batch_index, batch in enumerate(train_loader):
                ...
                with accelerator.autocast():
                    prepared = student.prepare_training_batch(
                        batch, teacher=self.ema_model,
                ...
                with accelerator.accumulate(self.model):
                    ...
                    components = getattr(student, "_last_flow_losses", {})
                    epoch_loss[2] += torch.as_tensor(components.get("fm_loss", loss.detach()), device=device).double() * batch["action"].shape[0]
                    epoch_loss[3] += torch.as_tensor(components.get("ct_loss", 0.), device=device).double() * batch["action"].shape[0]
                    self.global_step += 1
            ...
            log = {"epoch": self.epoch, "global_step": self.global_step,
                   ...
                   "train_fm_loss": float(epoch_loss[2] / epoch_loss[1]),
                   "train_ct_loss": float(epoch_loss[3] / epoch_loss[1]),
            ...
            actions_due = self.epoch % int(cfg.training.get("sample_every", 1)) == 0
            if validation_enabled and (validation_due or actions_due):
                log.update(self._validate(accelerator, val_loader, val_dataset,
                                         generated=bool(cfg.training.get("validate_generated_history", True)), compute_actions=actions_due))
            if not cfg.task.policy.get("lazy_eval", True) and self.epoch % int(cfg.training.rollout_every) == 0:
                ...
                accelerator.wait_for_everyone()
            score = log.get("val_expert_action_mse")
...
@hydra.main(version_base=None, config_path="../config", config_name="train_p2n_action_flow")
def main(cfg):
    TrainP2NActionFlowWorkspace(cfg).run()
