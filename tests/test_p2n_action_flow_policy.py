...
from oat.policy.p2n_action_flow_common import prepare_action_flow_training_batch
...
def make_flow(gate=False, *, queries=2, history_gate_mode='learned'):
    ...
    kwargs = dict(shape_meta=META, obs_encoder=obs,
        embed_dim=16, n_layers=2, n_heads=2, resampler_ffn_dim=32,
        flow=dict(ffn_hidden_dim=64),
    ...
    for key in ['action', *obs.state_ports]:
...
def test_packed_teacher_student_updates_and_layout(gate):
    ...
    for update_index in range(4):
        prepared=prepare_action_flow_training_batch(batch,student=student,teacher=teacher,
        ...
        if update_index == 3:
            assert all(p.grad is not None and torch.count_nonzero(p.grad) for p in student.parameters() if p.requires_grad)
...
def test_explicit_history_prediction_validation_and_acknowledgement(gate):
    ...
    assert metrics['action_mse']['count']==7
    assert metrics['normalized_action_mse']['count']==7
...
def test_offline_artifact_strict_roundtrip_and_family_rejection(gate,tmp_path):
    ...
    wrong=P2NActionFlowPolicy if gate else P2NStateGateActionFlowPolicy
...
def test_teacher_uses_own_adapter_and_one_shared_frozen_patch_realization(monkeypatch):
    ...
    import oat.policy.p2n_action_flow_common as common
    ...
    try:
        prepared=common.prepare_action_flow_training_batch(batch,student=student,teacher=teacher,
...
def test_nonidentity_normalizer_target_and_single_inverse(monkeypatch):
    policy=make_flow().eval()
    normalizer=copy.deepcopy(policy.action_normalizer)
    values=torch.randn(100,7)*3+8
    values[:,6]=.5  # constant command dimension still round trips
    ...
    assert torch.allclose(normalizer['action'].unnormalize(normalized),raw,atol=2e-6)
    monkeypatch.setattr(policy,'_generate_normalized_actions',lambda *a,**kw: normalized)
    result=policy._generate_actions({},None,None)
    assert torch.allclose(result,raw,atol=2e-6)
    assert not any('tokenizer' in n or 'latent_adapter' in n or 'quantizer' in n for n in policy.state_dict())
    assert not hasattr(policy,'action_tokenizer')
    with pytest.raises(TypeError):
        P2NActionFlowPolicy(shape_meta=META,tokenizer_checkpoint='/nonexistent')
    with pytest.raises(ValueError,match='continuous_action_flow'):
        from oat.policy.p2n_action_flow_common import validate_flow_config
        validate_flow_config({'levels':[8,5,5,5,5]})


def test_future_mask_never_changes_training_targets():
    student=make_flow().train(); student.self_past_p=0
    teacher=copy.deepcopy(student).requires_grad_(False).eval()
    batch=make_batch(student)
    other=copy.deepcopy(batch); other['future_action_valid'].zero_()
    a=prepare_action_flow_training_batch(batch,student=student,teacher=teacher,
        generator=torch.Generator().manual_seed(77))
    b=prepare_action_flow_training_batch(other,student=student,teacher=teacher,
        generator=torch.Generator().manual_seed(77))
    assert torch.equal(a.noisy_actions,b.noisy_actions)
    assert torch.equal(a.velocity_targets,b.velocity_targets)


def test_online_noise_advances_then_reset_repeats_episode():
    policy=make_flow().eval(); obs=policy.create_dummy_observation(1)
    a=policy.predict_action(obs)['action_pred']
    policy.record_executed_actions(torch.empty(1,0,7))
    b=policy.predict_action(obs)['action_pred']
    assert not torch.equal(a,b)
    policy.reset()
    c=policy.predict_action(obs)['action_pred']
