...
from oat.workspace.train_p2n_latent_flow import (
    MetricSums, NonPaddingDistributedSampler, TrainP2NLatentFlowWorkspace,
    assert_teacher_synchronized, make_fresh_ema, stable_validation_seed,
    successful_update, validate_optimizer_ownership,
)
...
def test_sample_noise_is_partition_and_order_independent():
    def noises(ids):
        return {index: torch.randn(8, 5, generator=torch.Generator().manual_seed(
...
def _cfg():
    return OmegaConf.create(dict(
        ...
        policy=dict(variant="p2n_latent_flow", flow=dict(inference_steps=8)),
...
def _payload(cfg):
    return dict(cfg=copy.deepcopy(cfg), policy_config=dict(construction_mode="restore"),
                metadata=dict(policy_family="oat_latent_flow", artifact_schema_version=1,
                              variant="p2n_latent_flow", task_type="real_robot"),
...
def test_resume_rejects_wrong_family_variant_loss_and_incomplete_state():
    ...
    assert TrainP2NLatentFlowWorkspace.validate_resume_payload(payload, cfg) is payload
    changed = copy.deepcopy(payload)
    changed["metadata"]["policy_family"] = "oat_autoregressive"
    with pytest.raises(ValueError, match="policy_family"):
        TrainP2NLatentFlowWorkspace.validate_resume_payload(changed, cfg)
    changed = copy.deepcopy(payload)
    changed["metadata"]["variant"] = "p2n_state_gate_latent_flow"
    ...
    with pytest.raises(ValueError, match="policy.flow"):
        TrainP2NLatentFlowWorkspace.validate_resume_payload(payload, changed_cfg)
    changed = copy.deepcopy(payload)
...
def test_dedicated_rng_state_resumes_every_stream_exactly():
    workspace = TrainP2NLatentFlowWorkspace(_cfg(), output_dir="unused")
...
def test_complete_artifact_restores_distinct_ema_optimizer_and_counters(monkeypatch, tmp_path):
    ...
    restored = TrainP2NLatentFlowWorkspace.create_from_checkpoint(path, output_dir=str(tmp_path))
...
def test_workspace_flushes_tail_and_advances_only_optimizer_updates(monkeypatch, tmp_path):
    ...
    import oat.workspace.train_p2n_latent_flow as module
