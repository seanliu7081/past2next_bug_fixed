...
CPU execution:
  /venv/real_robot/bin/python -m torch.distributed.run --standalone \
    --nproc_per_node=2 tests/p2n_action_flow_ddp_smoke.py
...
Real model (explicitly allocates the user's selected GPUs):
  CUDA_VISIBLE_DEVICES=2,3 /venv/real_robot/bin/python -m torch.distributed.run \
    --standalone --nproc_per_node=2 tests/p2n_action_flow_ddp_smoke.py \
    --real --variant p2n_action_flow --task real_robot --dino /local/dino/snapshot

Repeat --real with p2n_state_gate_action_flow for the other variant. Real mode
...
def toy_mode(args, rank, device):
    # Reuse the integration fixtures' tiny production modules and direct 16x7 actions.
    # Only the external frozen DINO backbone is replaced by a local mock.
    sys.path.insert(0, str(ROOT / "tests"))
    import transformers
    from test_p2n_new_policy import MockDINOBackbone
    from test_p2n_action_flow_policy import make_flow, make_batch
    transformers.DINOv3ViTModel = MockDINOBackbone
    student = make_flow(args.variant == "p2n_state_gate_action_flow").to(device)
...
def real_mode(args, rank, device):
    import hydra
    from scripts.train_p2n_action_flow import compose_config, validate_dino_source
    ...
    from oat.workspace.train_p2n_action_flow import _move
...
def main():
    ...
    parser.add_argument("--variant", choices=("p2n_action_flow", "p2n_state_gate_action_flow"), default="p2n_action_flow")
    ...
    try:
        ...
        if not args.real:
            ...
            # A full training continuation must reproduce the next packed
            # FM/CT loss, distributed gradients, Adam step, and EMA update.
            def continuation_update():
                wrapped.train()
                optimizer.zero_grad(set_to_none=True)
                prepared = prepare(student, teacher, generator, past_generator)
                next_loss = wrapped(prepared)
                next_loss.backward()
                torch.nn.utils.clip_grad_norm_(student.parameters(), 1.)
                optimizer.step()
                successful_update(student, ema, scheduler)
            ...
            expected_continuation = continuation_update()
            ...
            torch.set_rng_state(saved["torch_rng"])
            actual_continuation = continuation_update()
            assert expected_continuation == actual_continuation, (expected_continuation, actual_continuation)
